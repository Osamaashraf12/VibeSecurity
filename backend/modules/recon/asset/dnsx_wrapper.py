...
@vibe_tool(submodule="sub_bforcer", tool_id="dnsx", tool_name="DNSx", category="asset", ext="txt")
def run(ctx: ToolContext):
    output_file = ctx.get_output_path("dnsx", "asset")
...
def run_bruteforce(ctx: ToolContext, target_domain: str, output_file: str, wordlist: str):
    ...
    # This ensures paths are translated and the command runs inside Docker
    result = ctx.run_command(cmd)
    
    if result.returncode != 0:
        print(f"[!] DNSx returned a non-zero exit code: {result.stderr or result.stdout}")